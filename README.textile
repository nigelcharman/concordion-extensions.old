...
bc..      LoggingTooltipExtension.setLoggerNames("carbon.CarbonCalculatorTest", "selenium.events");
...
Adds screenshots to the Concordion output, typically when failures or exceptions occur. The screenshot is displayed when you hover over the relevant element, and clicking on the element will open the image in the current browser window. 
...
bc..      ScreenshotExtension.setMaxWidth(400);
...
bc..      EventFiringWebDriver driver = new EventFiringWebDriver(new FirefoxDriver());
